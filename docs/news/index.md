...
- replace cdsrmodels dependency with inline lin_associations
  implementation using WGCNA and ashr
...
- fix subscript out of bounds in
...
- update `pheatmap_qc` (change standardized concentrations into unique
  strings)
...
- update `heatmap_combo_with_isoref_panel` to work with not standard
  concentrations
...
- refactor `create_PRISM_plot_list_sa` and
...
- add download feature in `prep_plot_chunk` and
...
- protect against unexpected data aggregation in
...
- refactor `prep_plot_chunk` to support simple nested lists of
  visualizations
- refactor plate visualizations to improve the performance

## gDRplots 0.0.53 - 2024-12-10

- refactor handling 0 in concentration column and -Inf and Inf for
  metrics

## gDRplots 0.0.52 - 2024-12-09

- order subplots in `plot_scatter_with_corr_panel` according to given
  list
...
- refactor `heatmap_combo_metrics_panel` function - split in single plot
  and panel
...
- unify function name - rename `heatmap_combo_metrics` into
  `heatmap_combo_metrics_panel`

## gDRplots 0.0.45 - 2024-11-20

- add possibility for plotting isoline references for HSA Excess and
  Bliss Excess
...
- add clustering in `pheatmap_with_anno_sa` and
  `pheatmap_with_anno_combo` functions
...
- update plots for quality control of the distribution of normalized
  combo data
...
- fix the lack of color for dose-response curves for single-agent in a
  small set of data
...
- fix issue with app crash when limits in `create_color_palette` are the
  same

## gDRplots 0.0.12 - 2024-07-12

- move `get_combo_col_settings` and `get_iso_colors` from `gDRutils`
  package

## gDRplots 0.0.11 - 2024-07-11

- add plots to quality control for combo data - combo metrics and
  reference isoline
...
- add plots to quality control of distribution of normalized data and
  metric stat
...
- move plot function from `metricClustering`, `responseGrid` and
  `metricDistribution` packages
- move `get_metrics_to_transform`, `convert_factor_to_character` and
  `paletteBrew` from `gDRcomponents` package
