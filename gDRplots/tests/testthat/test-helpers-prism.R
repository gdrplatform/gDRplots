...
test_that("prep_dt_response_sa works as expected", {
  ...
  expect_equal(NROW(dt_response_sa), NROW(res_dose))
...
test_that("prep_dt_response_combo works as expected", {
  mae <- gDRutils::get_synthetic_data("combo_matrix_small")
  se <- mae[[gDRutils::get_supported_experiments("combo")]]
  dt_metrics <- gDRutils::convert_se_assay_to_dt(se = se,
                                                 assay_name = "Metrics")
  ...
  sel_met_combo <- c("hsa_score", "bliss_score")
  res_scores <- 
    dt_scores[get(drug_name) == d_name & get(drug_name_2) == d_name2 & normalization_type == "RV", ]
  
  sel_met <- c("xc50", "x_mean", "x_max")
  res_cotrt <- data.table::dcast(
    dt_metrics[get(drug_name) == d_name & get(drug_name_2) == d_name2 & 
                 normalization_type == "RV", ][!is.na(cotrt_value)], 
    formula = rId + cId + get(cellline_name) ~ cotrt_value + source, 
    value.var = "xc50")
  data.table::setnames(res_cotrt, "cellline_name", cellline_name)
  ls_col_diff <- setdiff(colnames(res_cotrt), meta_col)
  ls_col_diff_fin <- ls_col_diff[!grepl("0_", ls_col_diff)]
  comb <- c(c("cotrt_zero", "cotrt", "cotrt_diff"))
  ...
  # testing assertions
  expect_error(prep_dt_response_combo(dt_metrics = unlist(dt_metrics),
                                      dt_scores = dt_scores,
                                      d_name = d_name,
                                      d_name2 = d_name2),
               "Assertion on 'dt_metrics' failed: Must be a data.table")
  expect_error(prep_dt_response_combo(dt_metrics = dt_metrics,
