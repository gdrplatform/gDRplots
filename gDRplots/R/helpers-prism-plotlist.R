...
#' Validate inputs for PRISM plot list creation
#' @noRd
.validate_prism_inputs <- function(experiment_type, dt_metrics_sa, dt_metrics_combo,
                                   dt_average, dt_scores, drug1_name_vec, drug2_name_vec,
                                   normalization_type_vec, metric, metric_scores,
                                   fit_source, meta_data_path, feat_data_path,
                                   feature_sets, metadata_columns, clear_taxonomy_info) {
  ...
  if (experiment_type == "sa") {
    .validate_prism_inputs_sa(drug1_name_vec, dt_metrics_sa, dt_average, metric)
  } else {
    .validate_prism_inputs_combo(drug1_name_vec, drug2_name_vec, dt_metrics_combo,
                                dt_scores, metric, metric_scores)
  }
}

#' Validate single-agent specific inputs
#' @noRd
.validate_prism_inputs_sa <- function(drug1_name_vec, dt_metrics_sa, dt_average, metric) {
...
#' Validate combo specific inputs
#' @noRd
.validate_prism_inputs_combo <- function(drug1_name_vec, drug2_name_vec,
                                         dt_metrics_combo, dt_scores,
...
#' Prepare DepMap data sources (feature sets and metadata columns)
#' @noRd
.prep_depmap_sources <- function(metadata_columns, feat_data_path, feature_sets, meta_data_path) {
  ...
  list(metadata_columns = metadata_columns, feat_data_path = feat_data_path,
       feature_sets = feature_sets)
}

#' Build the drug name grid for iteration
#' @noRd
.prep_drug_grid <- function(experiment_type, drug1_name_vec, drug2_name_vec,
                            dt_metrics_sa, dt_metrics_combo, dt_average, dt_scores,
...
#' Build depmap items table from feature sets and metadata columns
#' @noRd
.build_depmap_items <- function(feature_sets, metadata_columns) {
...
#' Prepare response data for a single-agent iteration
#' @noRd
.prep_response_sa <- function(dt_metrics_sa, dt_average, d_name, norm,
                              metric, fit_source, id_col) {
  dt_met <-
  ...
  list(dt_response = dt_response, id_cols_to_remove = id_col)
}

#' Prepare response data for a combo iteration
#' @noRd
.prep_response_combo <- function(dt_metrics_combo, dt_scores, d_name, d_name2,
                                 norm, metric, metric_scores, fit_source,
                                 id_col, drug_name, drug_name_2) {
  dt_met_diff <-
...
.create_PRISM_plot_list <- function(experiment_type,
  ...
  .validate_prism_inputs(experiment_type, dt_metrics_sa, dt_metrics_combo,
                         dt_average, dt_scores, drug1_name_vec, drug2_name_vec,
                         normalization_type_vec, metric, metric_scores,
                         fit_source, meta_data_path, feat_data_path,
                         feature_sets, metadata_columns, clear_taxonomy_info)
  ...
  # depmap specific
  depmap_src <- .prep_depmap_sources(metadata_columns, feat_data_path,
                                     feature_sets, meta_data_path)
  metadata_columns <- depmap_src$metadata_columns
  feat_data_path <- depmap_src$feat_data_path
  feature_sets <- depmap_src$feature_sets
  ...
  # adjust drug list
  drug_name_grid <- .prep_drug_grid(experiment_type, drug1_name_vec, drug2_name_vec,
                                    dt_metrics_sa, dt_metrics_combo, dt_average, dt_scores,
                                    drug_name, drug_name_2)

  # fast end when there are no available drugs
  if (NROW(drug_name_grid) == 0) {
    msg <- if (experiment_type == "sa") {
  ...
  depmap_items <- .build_depmap_items(feature_sets, metadata_columns)
  ...
  for (i_feat_meta in seq_len(NROW(depmap_items))) {
    ...
    for (i_drug in seq_len(NROW(drug_name_grid))) {
      ...
      for (norm in normalization_type_vec) {
        resp <- if (experiment_type == "sa") {
          d_name <- drug_name_grid[[drug_name]][i_drug]
          .prep_response_sa(dt_metrics_sa, dt_average, d_name, norm,
                            metric, fit_source, id_col)
        } else {
          d_name <- drug_name_grid[[drug_name]][i_drug]
          d_name2 <- drug_name_grid[[drug_name_2]][i_drug]
          .prep_response_combo(dt_metrics_combo, dt_scores, d_name, d_name2,
                               norm, metric, metric_scores, fit_source,
                               id_col, drug_name, drug_name_2)
        }
        # 4th level - prep vis for each metric
        selected_metrics <- list(
          selected_metric = setdiff(names(resp$dt_response), resp$id_cols_to_remove)
        )

        obj_vol <- purrr::pmap(selected_metrics,
                               plot_volcano_assoc_panel,
                               dt_response = resp$dt_response,
                               dt_depmap = dt_depmap,
