...
For experiments with drug combinations, users can visualize the full response matrix in the panel by using **heatmap_combo_metrics** function. 

Users get at once a view with smooth value, HSA (highest single-agent) excess, and Bliss excess for selected normalization type across combinations of concentrations, with isobolograms overlaid. Further, the CI plot is also shown (the Combination Index is displayed at different ratios of the two drugs).
...
Note. It required dedicated input calculated with `prep_dt_assoc` function.

![](../inst/figure/plot_volcano.png)

![](../inst/figure/plot_volcano_meta.png)

User can also explore more detailed correlation between one selected feature and selected metric with use of **plot_scatter_with_corr** function.

Note. It required special dedicated calculated with `prep_dt_depmap_feat` function and one of functions
...
![](../inst/figure/plot_scatter_with_corr.png)

User can also explore more detailed distribution of selected metadate and selected metric with use of **plot_boxplot_meta** function.

![](../inst/figure/plot_boxplot_meta.png)

Note. It required special dedicated calculated with `prep_dt_depmap_meta` function and one of function
with experimental response data for one metric: `prep_dt_response_metric_sa`, `prep_dt_response_dose_sa`, `prep_dt_response_scores` or `prep_dt_response_metric_diff`. 
